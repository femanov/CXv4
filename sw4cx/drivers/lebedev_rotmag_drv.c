...
#include "drv_i/lebedev_rotmag_drv_i.h"
...
{
    [C_GO_DUMP] = {"outrb6", VDEV_PRIV, -1},
    [C_GO_RING] = {"outrb7", VDEV_PRIV, -1},
    [C_INPREG8] = {"inpr8b", VDEV_PRIV, -1},
    [C_I_MOTOR] = {"adc31",  VDEV_TUBE, LEBEDEV_ROTMAG_CHAN_I_MOTOR},
};
...
static void ReportStopReason(privrec_t *me, const char *reason)
{
  ...
  static int       ch_stop_reason = LEBEDEV_ROTMAG_CHAN_STOP_REASON;
...
static void DoStop (privrec_t *me, const char *format, ...)
{
  ...
  char     buf[100];
    ...
    SndCVal(me, C_GO_DUMP, 0);
    SndCVal(me, C_GO_RING, 0);
    ReturnInt32Datum(me->devid, LEBEDEV_ROTMAG_CHAN_GO_TIME_LEFT, 0, 0);
    ReturnInt32Datum(me->devid, LEBEDEV_ROTMAG_CHAN_GOING_DIR,    0, 0);
...
////    fprintf(stderr, "duration=%d\n", duration);
        ...
        ReturnInt32Datum(me->devid, LEBEDEV_ROTMAG_CHAN_GO_TIME_LEFT,
                         duration, 0);
    ...
    SndCVal(me, C_GO_DUMP, dir == 0? 1 : 0);
    SndCVal(me, C_GO_RING, dir == 1? 1 : 0);
    ReturnInt32Datum(me->devid, LEBEDEV_ROTMAG_CHAN_GOING_DIR, me->target, 0);
    ReportStopReason(me, "");
}

static void lebedev_rotmag_sodc_cb(int devid, void *devptr,
                                   int sodc, int32 val)
{
  privrec_t      *me = (privrec_t *)devptr;
  int             lim_ring;
  ...
  struct timeval  limit;
    ...
    if      (sodc == C_INPREG8)
    {
        ...
        if ((lim_dump  &&  me->target < 0)  ||
            ...
            DoStop(me, "%s limit switch reached", me->target < 0? "DUMP" : "RING");
    }
    else if (sodc == C_I_MOTOR)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                ReturnInt32Datum(devid, LEBEDEV_ROTMAG_CHAN_I_EXC_TIME, 0, 0);
            }
        }
    }
}

static int lebedev_rotmag_init_d(int devid, void *devptr,
                                 ...
                                 const char *auxinfo)
{
  ...
  int             pos;
    ...
    SetChanRDs(devid, LEBEDEV_ROTMAG_CHAN_I_EXC_TIME,   1, CHAN_TIME_R, 0);
    SetChanRDs(devid, LEBEDEV_ROTMAG_CHAN_N_S,          1, CHAN_TIME_R, 0);
...
//                      IS_AUTOUPDATED_TRUSTED);
    ...
    me->ctx.do_rw          = NULL;
    me->ctx.sodc_cb        = lebedev_rotmag_sodc_cb;

    return vdev_init(&(me->ctx), devid, devptr, -1, auxinfo);
}

static void lebedev_rotmag_term_d(int devid __attribute__((unused)), void *devptr)
{
  privrec_t      *me = (privrec_t *)devptr;
...
static void lebedev_rotmag_rw_p(int devid, void *devptr,
                                int action,
                                int count, int *addrs,
                                cxdtype_t *dtypes, int *nelems, void **values) 
{
  ...
  struct timeval  timediff;
    ...
    for (n = 0;  n < count;  n++)
    {
        ...
        if      (chn == LEBEDEV_ROTMAG_CHAN_I_LIM)
        {
            if (action == DRVA_WRITE)
            {
                if (val < 0)         val = 0;
        ...
        else if (chn == LEBEDEV_ROTMAG_CHAN_I_LIM_TIME)
        {
            if (action == DRVA_WRITE)
            {
                if (val < 0)               val = 0;
        ...
        else if (chn == LEBEDEV_ROTMAG_CHAN_GO_DUMP_MAX  ||
                 chn == LEBEDEV_ROTMAG_CHAN_GO_RING_MAX)
        {
            if (action == DRVA_WRITE  &&  val == CX_VALUE_COMMAND)
                DoStart(me, -1, chn - LEBEDEV_ROTMAG_CHAN_GO_MAX_d_base);
            ReturnInt32Datum(devid, chn, 0, 0);
        }
        else if (chn == LEBEDEV_ROTMAG_CHAN_GO_DUMP_100MS  ||
                 chn == LEBEDEV_ROTMAG_CHAN_GO_RING_100MS)
        {
            if (action == DRVA_WRITE  &&  val == CX_VALUE_COMMAND)
                DoStart(me, 0.1 * CHAN_TIME_R, chn - LEBEDEV_ROTMAG_CHAN_GO_100_d_base);
            ReturnInt32Datum(devid, chn, 0, 0);
        }
        else if (chn == LEBEDEV_ROTMAG_CHAN_GO_DUMP_N_S  ||
                 chn == LEBEDEV_ROTMAG_CHAN_GO_RING_N_S)
        {
            if (action == DRVA_WRITE  &&  val == CX_VALUE_COMMAND)
                DoStart(me, me->n_time, chn - LEBEDEV_ROTMAG_CHAN_GO_N_S_d_base);
            ReturnInt32Datum(devid, chn, 0, 0);
        }
        else if (chn == LEBEDEV_ROTMAG_CHAN_N_S)
        {
            if (action == DRVA_WRITE)
            {
                if (val < 1)                 val = 1;
        ...
        else if (chn == LEBEDEV_ROTMAG_CHAN_STOP)
        {
            if (action == DRVA_WRITE  &&  val == CX_VALUE_COMMAND)
                DoStop(me, "STOP command");
            ReturnInt32Datum(devid, chn, 0, 0);
        }
        else if (chn == LEBEDEV_ROTMAG_CHAN_I_EXC_TIME)
        {
            if (me->time_exc_start.tv_sec == 0)
                ReturnInt32Datum(devid, chn, 0, 0);
            else
...
//fprintf(stderr, "%ld.%06ld - %ld.%06ld = %ld.%06ld\n", now.tv_sec, now.tv_usec, me->time_exc_start.tv_sec, me->time_exc_start.tv_usec, timediff.tv_sec, timediff.tv_usec);
        ...
        else if (chn == LEBEDEV_ROTMAG_CHAN_GO_TIME_LEFT)
        {
            gettimeofday(&now, NULL);
            ReturnInt32Datum(devid, chn,
        ...
        else if (chn == LEBEDEV_ROTMAG_CHAN_GOING_DIR)
        {
...
DEFINE_CXSD_DRIVER(lebedev_rotmag, "Lebedev's Rotating Magnet driver",
                   NULL, NULL,
