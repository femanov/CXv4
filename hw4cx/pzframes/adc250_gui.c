...
static Widget adc250_mkctls(pzframe_gui_t           *gui,
                            ...
                            pzframe_gui_mkparknob_t  mkparknob)
{
  ...
  char    spec[1000];
    ...
    MakeParKnob(dwgrid, 1, y++,
                ...
                gui, mkparknob, ADC250_CHAN_WAITTIME);
    ...
    snprintf(spec, sizeof(spec),
             ...
             1, ADC250_MAX_NUMPTS, 1, ADC250_MAX_NUMPTS);
    w1 = mkparknob(gui, ptsform, spec, ADC250_CHAN_NUMPTS);
    snprintf(spec, sizeof(spec),
             " look=text rw label='@' step=100"
             " options='withlabel'"
             " minnorm=%d maxnorm=%d mindisp=%d maxdisp=%d dpyfmt='%%6.0f'",
             0, ADC250_MAX_NUMPTS-1, 0, ADC250_MAX_NUMPTS-1);
    w2 = mkparknob(gui, ptsform, spec, ADC250_CHAN_PTSOFS);
    attachleft(w2, w1, 0);
    ...
    for (nl = 0;  nl < ADC250_NUM_LINES;  nl++)
    {
        ...
        MakeParKnob    (pcgrid, 2, nl,
                        ...
                        gui, mkparknob, ADC250_CHAN_RANGE0 + nl);
        MakeParKnob    (pcgrid, 3, nl,
                        " look=led ro label='' options='color=red'",
                        gui, mkparknob, ADC250_CHAN_OVERFLOW0 + nl);
    ...
    MakeParKnob(vergrid, 1, y, "look=text ro dpyfmt=%-5.0f", gui, mkparknob, ADC250_CHAN_BASE_SW_VER);
    MakeParKnob(vergrid, 2, y, "look=text ro dpyfmt=%-5.0f", gui, mkparknob, ADC250_CHAN_PGA_SW_VER);
    y++;
    MakeALabel (vergrid, 0, y, "HW ver");
    MakeParKnob(vergrid, 1, y, "look=text ro dpyfmt=%-5.0f", gui, mkparknob, ADC250_CHAN_BASE_HW_VER);
    MakeParKnob(vergrid, 2, y, "look=text ro dpyfmt=%-5.0f", gui, mkparknob, ADC250_CHAN_PGA_HW_VER);
    y++;
    MakeALabel (vergrid, 0, y, "Uniq ID");
    MakeParKnob(vergrid, 1, y, "look=text ro dpyfmt=%-5.0f", gui, mkparknob, ADC250_CHAN_BASE_UNIQ_ID);
...
pzframe_gui_dscr_t *adc250_get_gui_dscr(void)
{
  ...
  static int                 dscr_inited;
    ...
    if (!dscr_inited)
    {
        FastadcGuiFillStdDscr(&dscr,
                              pzframe2fastadc_type_dscr(adc250_get_type_dscr()));
