...
#if 0
    vme_hal_a32rd32(the_bus_handle, dev.am, dev.base_addr + dev.epcs_offs + offs, v_p);
#else
    if    (dev.data_size == 32)
    {
        if      (dev.addr_size == 32) vme_hal_a32rd32(the_bus_handle, dev.am, dev.base_addr + dev.epcs_offs + offs, v_p);
        else if (dev.addr_size == 24) vme_hal_a24rd32(the_bus_handle, dev.am, dev.base_addr + dev.epcs_offs + offs, v_p);
        else   /*dev.addr_size == 16*/vme_hal_a16rd32(the_bus_handle, dev.am, dev.base_addr + dev.epcs_offs + offs, v_p);
    }
    else /*dev.data_size == 16*/
    {
        // Note: VME is BIG-endian, so that HI-word is at offset 0 inside 32bits and LO-word is at offset 2
        if      (dev.addr_size == 32)
        {
            vme_hal_a32rd16(the_bus_handle, dev.am, dev.base_addr + dev.epcs_offs + offs + 0, &wh);
            vme_hal_a32rd16(the_bus_handle, dev.am, dev.base_addr + dev.epcs_offs + offs + 2, &wl);
        }
        else if (dev.addr_size == 24)
        {
            vme_hal_a24rd16(the_bus_handle, dev.am, dev.base_addr + dev.epcs_offs + offs + 0, &wh);
            vme_hal_a24rd16(the_bus_handle, dev.am, dev.base_addr + dev.epcs_offs + offs + 2, &wl);
        }
        else   /*dev.addr_size == 16*/
        {
            vme_hal_a16rd16(the_bus_handle, dev.am, dev.base_addr + dev.epcs_offs + offs + 0, &wh);
            vme_hal_a16rd16(the_bus_handle, dev.am, dev.base_addr + dev.epcs_offs + offs + 2, &wl);
        }
        *v_p = (((uint32)wh) << 16) | wl;
    }
#endif
...
{
#if 0
    vme_hal_a32wr32(the_bus_handle, dev.am, dev.base_addr + dev.epcs_offs + offs, v);
#else
    if    (dev.data_size == 32)
    {
        if      (dev.addr_size == 32) vme_hal_a32wr32(the_bus_handle, dev.am, dev.base_addr + dev.epcs_offs + offs, v);
        else if (dev.addr_size == 24) vme_hal_a24wr32(the_bus_handle, dev.am, dev.base_addr + dev.epcs_offs + offs, v);
        else   /*dev.addr_size == 16*/vme_hal_a16wr32(the_bus_handle, dev.am, dev.base_addr + dev.epcs_offs + offs, v);
    }
    else /*dev.data_size == 16*/
    {
        // Note: VME is BIG-endian, so that HI-word is at offset 0 inside 32bits and LO-word is at offset 2
        if      (dev.addr_size == 32)
        {
            vme_hal_a32wr16(the_bus_handle, dev.am, dev.base_addr + dev.epcs_offs + offs + 0, (v >> 16) & 0xFFFF);
            vme_hal_a32wr16(the_bus_handle, dev.am, dev.base_addr + dev.epcs_offs + offs + 2,  v        & 0xFFFF);
        }
        else if (dev.addr_size == 24)
        {
            vme_hal_a24wr16(the_bus_handle, dev.am, dev.base_addr + dev.epcs_offs + offs + 0, (v >> 16) & 0xFFFF);
            vme_hal_a24wr16(the_bus_handle, dev.am, dev.base_addr + dev.epcs_offs + offs + 2,  v        & 0xFFFF);
        }
        else   /*dev.addr_size == 16*/
        {
            vme_hal_a16wr16(the_bus_handle, dev.am, dev.base_addr + dev.epcs_offs + offs + 0, (v >> 16) & 0xFFFF);
            vme_hal_a16wr16(the_bus_handle, dev.am, dev.base_addr + dev.epcs_offs + offs + 2,  v        & 0xFFFF);
        }
    }
#endif
...
static int readflash_proc(const char *arg __attribute__((unused)), int fd)
{
  ...
  enum {MAX_TRIES = 100};
    ...
    {
        if (option_verbose) fprintf(stderr, "\rReading from offset 0x%08x ", read_offs);
...
//            if (option_verbose) fprintf(stderr, "Checking (EPCS_CMD & EPCS_CMD_RD) try %d\r", try_n);
        ...
        {
            write4b(EPCS_OFS_EPCS_BUFF_RD_ADDR, 0); // Set read address for non-auto-incrementing devices (those with D16, namely the S_TIMER)
            read4b(EPCS_OFS_EPCS_BUFF_DATA, &w);
            // Bit-flip the word read...
            w = mirror32bits(w);
            // ...and disassemble it into bytes in an endian-agnostic way...
            bytes[0] = (w >> 24) & 0xFF;
            bytes[1] = (w >> 16) & 0xFF;
            bytes[2] = (w >>  8) & 0xFF;
            bytes[3] =  w        & 0xFF;
            // Finally, write it into file
...
static int writeflash_proc(const char *arg __attribute__((unused)), int fd)
{
  ...
  enum {MAX_TRIES = 100};
    ...
    {
        if (option_verbose) fprintf(stderr, "\rWriting to offset 0x%08x ", write_offs);
        ...
        {
            bytes[0] = bytes[1] = bytes[2] = bytes[3]; // Pre-fill everything with zeroes
            read_r = read(fd, bytes, sizeof(bytes));
            if (read_r < sizeof(bytes)) end_of_file = 1;
            if (read_r <= 0) goto END_READ_BLOCK;
            // Assemble the word-to-write in an endian-agnostic way...
            w =           bytes[3]         |
                (((uint32)bytes[2]) <<  8) |
                (((uint32)bytes[1]) << 16) |
                (((uint32)bytes[0]) << 24);
            // ...and bit-flip it
            w = mirror32bits(w);
            // Finally, write it to flash (with prior setting the write address)
