...
static void RlsMonSlot(int id)
{
  moninfo_t *mp = AccessMonSlot(id);
    ...
    if (mp->cpid >= 0)
    {
        ...
        fe_epics_periodics_needs_rebuild = 1;
    }
...
static void SendMonitor(int mid)
{
  moninfo_t      *mp    = AccessMonSlot(mid);
    ...
    cxsd_fe_epics_meat_update(mid, mp->PV_ptr);
}
                      ...
                      void          *privptr2)
{
  ...
  double         rds_buf[RDS_MAX_COUNT*2];
    ...
    {
        SendMonitor(id);
        //cxsd_fe_epics_meat_update(id, mp->PV_ptr);
    }
...
fprintf(stderr, "\tgcid=%d cpid=%d\n", gcid, cpid);
    ...
    {
        *mid_p = mid;
        return CXSD_FE_EPICS_ALREADY;
    }

    /* Check compatibility */
    if (!IsSuitableForExport(gcid)) return CXSD_FE_EPICS_ERROR;
...
static int RequestSubscription(void)
{
  cxsd_gchnid_t *wp;

    if (fe_epics_periodics_used == 0) return 0;

    if (fe_epics_periodics_needs_rebuild)
    {
        wp = fe_epics_periodics;
        ForeachMonSlot(RecordAMonitor, &wp);
        fe_epics_periodics_needs_rebuild = 0;
    }

//fprintf(stderr, "  fe_epics_periodics_used=%d\n", fe_epics_periodics_used);
    CxsdHwDoIO(0/*!!!ID!!!*/, DRVA_READ,
               fe_epics_periodics_used, fe_epics_periodics,
               NULL, NULL, NULL);
